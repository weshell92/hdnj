...
export default async function ChapterPage({ params }: { params: { book: BookId; chapterId: string } }) {
  const chapter = getChapter(params.book, params.chapterId)
  if (!chapter) notFound()
  const adjacent = getAdjacentChapters(params.book, params.chapterId)
  const externalOriginalText = await getCtextOriginalText(chapter)
  return (
    <article className="mx-auto max-w-4xl space-y-8">
      <div className="rounded-[2rem] bg-white/60 p-8 shadow-soft">
        <p className="text-sm font-bold tracking-[0.25em] text-cinnabar">{bookNames[chapter.book]} · 第 {chapter.chapterNumber} 篇</p>
        ...
        {chapter.sourceUrl ? <a className="mt-6 inline-flex rounded-full bg-ink px-5 py-3 text-sm font-semibold text-paper" href={chapter.sourceUrl} target="_blank" rel="noreferrer">查看 CText 原典来源</a> : null}
      </div>
      <ClassicReader chapter={chapter} externalOriginalText={externalOriginalText} />
